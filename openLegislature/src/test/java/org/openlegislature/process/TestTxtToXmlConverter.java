...
public class TestTxtToXmlConverter {
	...
	public void testInterjectionIsNotDiscoveredAtEndOfDoc01010() throws Exception {
		...
		testable.convertToXml(new File("src/test/resources/protocols-txt/01012.cleaned.txt"), new File("target/test-classes/01012.cleaned.xml"));
		testable.convertToXml(new File("src/test/resources/protocols-txt/01018.cleaned.txt"), new File("target/test-classes/01018.cleaned.xml"));
		...
		assertEquals(IOUtils.toString(new FileInputStream(new File("src/test/resources/protocols-txt/01012.cleaned.xml"))), IOUtils.toString(new FileInputStream(new File("target/test-classes/01012.cleaned.xml"))));
		assertEquals(IOUtils.toString(new FileInputStream(new File("src/test/resources/protocols-txt/01018.cleaned.xml"))), IOUtils.toString(new FileInputStream(new File("target/test-classes/01018.cleaned.xml"))));
